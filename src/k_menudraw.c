...
void M_DrawCupSelect(void)
{
	...
	V_DrawFill(0, 146 + ty, vid.width, 54, 31);
	M_DrawCupPreview(146 + ty, &templevelsearch);

	M_DrawCupTitle(120 - ty, &templevelsearch, 0);
	...
	switch (ref->type)
	{
		case SECRET_SKIN:
		{
			...
			if (skin != -1)
			{
				...
				M_DrawCharacterIconAndEngine(4, BASEVIDHEIGHT-(4+16), i, colormap, skin, 0);
			}
			break;
		}
		case SECRET_FOLLOWER:
